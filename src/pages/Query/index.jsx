...
function Query() {
  ...
  useEffect(() => {
    async function fetchContacts() {
      try {
        const response = await fetch(`${config.baseUrl}/api/contacts`);
  ...
  //   }
  //   fetch(
  //     `${config.baseUrl}/api/query?search=${search}&sort=${sort}&skip=${skip}&datemin=${startDateTemp}&datemax=${endDateTemp}`
  //   )
  //     .then((response) => response.json())
  ...
  const showModal = (id) => {
    ...
    modalRef.current.classList.add("opacity");
          ...
          <div className=" max-w-min-content mb-[100px]  col-span-full xl:col-span-12   ml-[45px]">
            <header className="px-5 py-6 border-b border-slate-100">
              <HeadBar
                title={"User Queries"}
                description={
                  "User queries from the user website contact form"
                }
                exportCSV={exportCSV}
                isVisible={true}
              />
              ...
              <div className="relative p-4 w-full max-w-md h-full md:h-auto">
                <div className="relative p-4 text-center bg-white rounded-lg shadow-md border border-slate-200 sm:p-5 ">
                  ...
                  Resolved requests will be deleted from the database. Are you
                    sure you want to delete this user query?
                  </p>
                  ...
                  <>
                    <thead className="text-xs font-semibold uppercase text-slate-600 bg-slate-200">
                      <tr>
                        ...
                        <th className="p-2 whitespace-nowrap">
                          <div className="font-bold text-left">Subject</div>
                        </th>
                        <th className="p-2 whitespace-nowrap">
                          <div className="font-bold text-left">Query</div>
                        </th>
                        <th className="p-2 whitespace-nowrap">
                          <div className="font-bold text-left">Registered on</div>
                        </th>
                    ...
                    {contacts !== "" &&
                      contacts.map((contact) => {
                        return (
                          <>
                            <React.Fragment>
                              <tr className="tr-main w-[100]" key={contact._id}>
                                <td className="p-2 whitespace-nowrap">
                                  <div className="text-left">
                                    <button
                                      type="button"
                                      className=" bg-red-200 border-red-500 border rounded-full text-red-500 cursor-default  font-semibold text-sm px-3 py-1  "
                                    >
                                      Unresolved
                                    </button>
                                  </div>
                                ...
                                <td className="p-2 whitespace-nowrap">
                                  <div className="text-left">{contact.email}</div>
                                </td>
